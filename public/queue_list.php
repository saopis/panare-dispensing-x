...
if($pay==1||$pay==""){
	...
	$condition2="and payed is not NULL and not_response is NULL";
	}
else if($pay==3) {
...
<?php if ($totalRows_rs_q_list > 0) { // Show if recordset not empty ?>
  <table class="thfont font14 table table-bordered table-hover">
    <tr >
      <td width="5%" align="center" bgcolor="#F4F4F4">Q</td>
      <td width="15%" align="center" bgcolor="#F4F4F4">HN</td>
      <td width="20%" align="center" bgcolor="#F4F4F4">ชื่อผู้ป่วย</td>
      <td width="10%" align="center" bgcolor="#F4F4F4">เวลาวางบัตร</td>
      <td width="10%" align="center" bgcolor="#F4F4F4">เวลาจ่าย</td>
      <td width="30%" align="center" bgcolor="#F4F4F4">ช่องบริการปัจจุบัน</td>
      <td width="5%" align="center" bgcolor="#F4F4F4">ยกเลิก</td>
  ...
  ?>
      <tr  style="cursor:pointer"   >
        <td align="center" class="thfont font16 font_bord" onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')"><?php echo $row_rs_q_list['queue']; ?></td>
        <td  align="center" onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')"><?php echo $row_rs_q_list['hn']; ?></td>
        <td align="center"  onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')"><?php echo $row_rs_q_list['ptname']; ?></td>
        <td  align="center" onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')"><?php echo substr($row_rs_q_list['queue_datetime'],10,9); ?></td>
        <td onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')" align="center" style="color:#FF0000; font-size:18px;" class="font_bord">&nbsp;</td>
        <td  align="center" style="color:#FF0000; font-size:18px;" class="font_bord" onclick="detail_load('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>','hn'); q_list('<?php echo $row_rs_q_list['hn']; ?>','<?php echo $curdate; ?>')"><?php if(dateDifference($row_rs_q_list['recent_rx_queue_datetime'], $row_channel['datenow'])<=30){ echo $row_rs_q_list['channel_name']; } ?></td>
        <td align="center" ><?php if($row_rs_q_list['q_delete']!='Y'){ ?><i class="fas fa-user-slash text-danger font20" onclick="if(confirm('ต้องการลบคิวนี้ใช่ไหม')==true){page_load('rx_queue','queue_list.php?queue=<?php echo $row_rs_q_list['queue']; ?>&vn=<?php echo $row_rs_q_list['vn']; ?>&do=delete'); }"></i><?php } else { ?> <i class="fas fa-user-check font20 text-success" onclick="if(confirm('ต้องการลบคิวนี้ใช่ไหม')==true){page_load('rx_queue','queue_list.php?queue=<?php echo $row_rs_q_list['queue'];  ?>&vn=<?php echo $row_rs_q_list['vn']; ?>&do=cancel');  }"></i><?php  } ?></td>
        <td align="center" ><input name="q_delete" type="button" onclick="if(confirm('ต้องการลบคิวนี้ใช่ไหม')==true){page_load('rx_queue','queue_list.php?queue=<?php echo $row_rs_q_list['queue']; ?>&vn=<?php echo $row_rs_q_list['vn']; ?>&do=delete2'); }" class="btn btn-danger"  value="ลบคิว" /></td>
      </tr>
      <?php
