...
$insert = "insert into ".$database_kohrx.".kohrx_had (icode) value ('".$_GET['drug']."')";
$rs_insert = mysql_query($insert, $hos) or die(mysql_error());

//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','insert into ".$database_kohrx.".kohrx_had (icode) value (\'".$_GET['drug']."\')')";
$rs_insert_log = mysql_query($insert_log, $hos) or die(mysql_error());
}

if($_GET['action']=="delete"){
mysql_select_db($database_hos, $hos);
$delete = "delete from ".$database_kohrx.".kohrx_had where icode='".$_GET['icode']."'";
$rs_delete = mysql_query($delete, $hos) or die(mysql_error());

//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','delete from ".$database_kohrx.".kohrx_had where icode=\'".$_GET['icode']."\'')";
$rs_insert_log = mysql_query($insert_log, $hos) or die(mysql_error());

mysql_select_db($database_hos, $hos);
$delete = "delete from ".$database_kohrx.".kohrx_had_cause where icode='".$_GET['icode']."'";
$rs_delete = mysql_query($delete, $hos) or die(mysql_error());

//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','delete from ".$database_kohrx.".kohrx_had_cause where icode=\'".$_GET['icode']."\'')";
$rs_insert_log = mysql_query($insert_log, $hos) or die(mysql_error());

}

mysql_select_db($database_hos, $hos);
$query_rs_drug_list = "SELECT concat(d.name,d.strength) as drugname,d.icode from ".$database_kohrx.".kohrx_had k left outer join drugitems d on d.icode=k.icode order by name";
...
function load_modal(id,drugname){
                $("#modal-body").load('drug_setting_had_edit.php?icode='+id, function(responseTxt, statusTxt, xhr){
...
<?php if ($totalRows_rs_drug_list > 0) { // Show if recordset not empty ?>
  <table width="100%" border="0" cellpadding="3" cellspacing="0"  id="tables" class="table table-striped table-bordered table-sm table-hover " style="width:100%; ">
	<thead>
      ...
      <td width="38" align="center" class="notexport">&nbsp;</td>
    </tr>
	</thead>
    <tbody>
	  <?php $i=0; do { $i++; 
...
mysql_select_db($database_hos, $hos);
$query_rs_cause = "SELECT use_cause from ".$database_kohrx.".kohrx_had_cause where icode='".$row_rs_drug_list['icode']."'";
    ...
    <tr class="grid">
      ...
      <td >
        <?php if ($totalRows_rs_cause > 0) { // Show if recordset not empty ?>
          <?php $i=0; do { $i++; ?>
            <div class="text-left">
              <?php echo "<span class='badge badge-secondary'>".$i."</span> ".$row_rs_cause['use_cause']; ?>
            </div>
          <?php } while ($row_rs_cause = mysql_fetch_assoc($rs_cause)); ?>
		  <?php } // Show if recordset not empty ?>
      </td>
      <td align="center" valign="top" class="notexport"><button onClick="load_modal('<?php echo $row_rs_drug_list['icode']; ?>','<?php echo $row_rs_drug_list['drugname']; ?>');" style="cursor: pointer" class="btn btn-success btn-sm   ">แก้ไขเหตุผล</button>&ensp;<button class="btn btn-sm btn-danger"><i class="fas fa-trash-alt font20" style="cursor: pointer" onClick="if(confirm('ต้องการลบข้อมูลนี้จริงหรือไม่?')==true){had_drug_delete('<?php echo $row_rs_drug_list['icode']; ?>');}"></i></button></td>
    </tr>      
