...
$query_insert = "insert into ".$database_kohrx.".kohrx_drug_protect_from_light (icode) value ('$lasa_drug')";
$rs_insert = mysql_query($query_insert, $hos) or die(mysql_error());
}
if(isset($_GET['do'])&&$_GET['do']=="spacial_delete"){
mysql_select_db($database_hos, $hos);
$query_insert = "delete from ".$database_kohrx.".kohrx_drug_protect_from_light where icode ='$icode'";
$rs_insert = mysql_query($query_insert, $hos) or die(mysql_error());
}

mysql_select_db($database_hos, $hos);
$query_rs_sp1 = "SELECT concat(d.name,' ',d.strength) as drugname,s.icode FROM ".$database_kohrx.".kohrx_drug_protect_from_light s left outer join s_drugitems d on s.icode=d.icode order by d.name ASC";
...
mysql_select_db($database_hos, $hos);
$query_rs_drug2 = "SELECT icode,concat(name,strength) as drugname FROM s_drugitems WHERE icode like '1%' and istatus='Y' and icode not in (select icode from ".$database_kohrx.".kohrx_drug_protect_from_light) ORDER BY name ASC";
...
<div class="p-3">
<div class="card">
<div class="card-header">กำหนดยาที่ต้องเก็บให้พ้นแสง</div>
<div class="card-body">
<form id="form1" name="form1" method="post" action="drug_setting_protect_from_light.php" class="form-group">
<div class="row">
 ...
 <?php if ($totalRows_rs_sp1 > 0) { // Show if recordset not empty ?>
  <table id="tables" width="100%" border="0" class="table table-sm table-striped table-hover ">
    ...
    <?php $i=0; do { $i++; 
      ...
      <td align="center" class="notexport" ><button class="btn btn-sm btn-danger" onClick="if(confirm('ต้องการลบข้อมูลนี้จริงหรือไม่?')==true){window.location='drug_setting_protect_from_light.php?do=spacial_delete&amp;icode=<?php echo $row_rs_sp1["icode"]; ?>';}">ลบ</button></td>
