...
$totalRows_s_patient = mysql_num_rows($s_patient);	
	...
	if($_POST['action']=="save"){
		...
		mysql_select_db($database_hos, $hos);
		$query_insert = "insert into ".$database_kohrx.".kohrx_molnupiravir_use (hn,use_date,use_time,qty,in_out_method,doctor,remark) value ('".$hn."','".date_th2db($use_date)."','".$use_time.":00',".$qty.",'".$in_out."','".$doctor."','".$remark."') ";
		$insert = mysql_query($query_insert, $hos) or die(mysql_error());
		echo "<script>page_load2('left-app','app_molnupiravir_register_record.php');page_load('main-app','app_molnupiravir_register_list.php');</script>";
	}
}
...
if($_POST['id']!=""){
mysql_select_db($database_hos, $hos);
	$query_rs_edit = "select * from ".$database_kohrx.".kohrx_molnupiravir_use where id='".$_POST['id']."'";
...
$(document).ready(function () {
  ...
  /*Esse foi o melhor script que consegui fazer, porem ele não permite backspace, delete nem arrows, o que torna a experiência ruim*/
				...
				$('#edit').click(function(){
					...
					type: "POST",
					data : {id:$('#id').val(),use_date:$('#use_date2').val(),use_time:$('#use_time2').val(),in_out:$('#in_out2').val(),qty:$('#qty2').val(),doctor:$('#doctor').val(),remark:$('#remark').val(),datestart:$('#datestart').val(),dateend:$('#dateend').val()},
					success: function(data, textStatus, jqXHR)
					{
						//data - response from server
								//data - response from server
	...
	///////////////////////////////
				$('#receive-save').click(function(){
					...
					url : "app_molnupiravir_register_list.php",
					type: "POST",
					data : {action:'receive',use_date:$('#use_date3').val(),use_time:$('#use_time3').val(),qty:$('#qty3').val(),datestart:$('#datestart').val(),dateend:$('#dateend').val()},
					success: function(data, textStatus, jqXHR)
					{
						//data - response from server
								//data - response from server
						page_load2('left-app','app_molnupiravir_register_record.php');
						$('#main-app').html(data);
						swal.close();
...
<nav class="navbar navbar-light text-white" style="background-color: #6D9384">
			...
			<div class="col-sm-auto">
				...
				<div class="row mt-2">
					...
					<div class="col-sm-auto">
						...
						<button class="btn btn-sm btn-warning" id="receive-cancel" onClick="page_load2('left-app','app_molnupiravir_register_record.php');">ยกเลิก</button>
