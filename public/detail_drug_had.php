...
if(isset($_POST['delete'])&&$_POST['delete']=="ลบ"){
	...
	$query_delete = "delete from ".$database_kohrx.".kohrx_had_record where id='$id'";
	$rs_delete = mysql_query($query_delete, $hos) or die(mysql_error());

//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','delete from ".$database_kohrx.".kohrx_had_record where id=\'".$id."\'')
";
...
foreach ($cause_id as $question){
	...
	mysql_select_db($database_hos, $hos);
	$query_insert = "insert into ".$database_kohrx.".kohrx_had_record (icode,".$field.",doctor,use_cause_id,remark) value ('$icode','".$str."','$doctor','$question','$remark')";
	$rs_insert = mysql_query($query_insert, $hos) or die(mysql_error());
	echo $query_insert;
	
	//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','insert into ".$database_kohrx.".kohrx_had_record (icode,".$field.",doctor,use_cause_id,remark) value (\'".$icode."\',\'".$str."\',\'".$doctor."\',\'".$question."\',\'".$remark."\')')";
$rs_insert_log = mysql_query($insert_log, $hos) or die(mysql_error());

	}
	else{
	mysql_select_db($database_hos, $hos);
	$query_update = "update ".$database_kohrx.".kohrx_had_record  set use_cause_id='$question' ,remark='$remark' where id='$id'";
	$rs_update = mysql_query($query_update, $hos) or die(mysql_error());

	//insert replicate_log		
mysql_select_db($database_hos, $hos);
$insert_log = "insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','update ".$database_kohrx.".kohrx_had_record  set use_cause_id=\'".$question."\' ,remark=\'".$remark."\' where id=\'".$id."\'')";
$rs_insert_log = mysql_query($insert_log, $hos) or die(mysql_error());
...
mysql_select_db($database_hos, $hos);
$query_rs_drug = "select c.use_cause,d.icode,concat(d.name,d.strength) as drugname,c.id from drugitems d left outer join ".$database_kohrx.".kohrx_had_cause c on c.icode=d.icode where d.icode='$icode'";
...
mysql_select_db($database_hos, $hos);
$query_rs_search = "select * from ".$database_kohrx.".kohrx_had_record where ".$field1."='".$str1."' and icode='$icode'";
...
<nav class="navbar navbar-dark bg-info text-white " style="padding-bottom: 10px; height: 50px;" >
  ...
  <ul class="navbar-nav">
    <li class="nav-item">
      <i class="fas fa-list-alt font20"></i>&ensp;บันทึกเหตุผลการใช้ยาเสี่ยงสูง (HIGH ALERT DRUG)
	  </li>
  </ul>
...
<form id="form1" name="form1" method="post" action="">
  ...
  <table width="100%" border="0" class="font14" cellspacing="0" cellpadding="0">
    <?php do { ?>
      <tr>
        <td><input <?php if (!(strcmp($row_rs_search['use_cause_id'],$row_rs_drug['id']))) {echo "checked=\"checked\"";} ?> name="cause_id[]" type="radio" id="cause_id[]" value="<?php echo $row_rs_drug['id']; ?>" />
          &nbsp; <span class="table_head_small"><?php echo $row_rs_drug['use_cause']; ?></span></td>
