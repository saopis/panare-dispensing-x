...
mysql_select_db($database_hos, $hos);
$query_rs_list1 = "SELECT t1.patient_name,t1.hn,t1.patient_name,t1.call_datetime,q.queue,n.channel_name,n.q_show,t1.main_dep_queue,kk.department
...
left outer join kskdepartment kk on kk.depcode=t1.main_dep
...
<div style="height:296px;" class="container4">
				...
				<div style="font-size:400px; margin-top:-185px; color:#FFFFFF;" class=" thsan-semibold"><?php if($row_rs_list1['q_show']=='Y'){ print $row_rs_list1['main_dep_queue']; } ?></div>			
                </div>
                <div class="right4">
	...
	$totalRows_selpic = mysql_num_rows($selpic);
		...
		<div style="color:#D51E42; font-size:100px; margin-top:-25px;" class="thfont font_border"><?php echo $row_rs_list1['channel_name']; ?></span></div>
                    <div style="font-size:25px; margin-top:-55px; color:#999" class="thfont font_border" >แผนกหลัก : <?php echo $row_rs_list1['department']; ?></div>       
                    </div>                    
                </div>
                </div>
            </div>
...
<!-- // คนที่รอเรียก // -->
<?php if($totalRows_rs_max<>0){ ?>            
<div style="background-color:#000; color: #FFF; font-size:50px; height:70px; padding-top:10px;" ><span style=" padding-left:20px;">คิวที่รอเรียก</span></div>
<div style="background-color: #666; height:47px;">
	 <div class="container2" style="height:47px;">
        ...
        <div class="right2" style="background-color: #999">
        	<div class="container3" style="height:47px;">
            <div class="left3"></div>
            <div class="right3">
            <div class="container6" style="height:47px;">
  ...
  if($row_channel['q_dep_type']=="main_dep"){
  	mysql_select_db($database_hos, $hos);
	$query_rs_q_order = "select p.hn,o.main_dep_queue,concat(p.pname,p.fname,' ',p.lname) as ptname,k.department from ovst o left outer join patient p on p.hn=o.hn left outer join kskdepartment k on k.depcode=o.main_dep where o.vstdate=CURDATE() and o.main_dep='".$row_channel['depcode']."' and o.main_dep_queue > '".$row_rs_max['main_dep_queue']."' order by o.main_dep_queue ASC limit 4";
    ...
    <div class="left2"  align="center">
	  ...
	  <?php  print $row_rs_q_order['main_dep_queue'];  ?>
    </div>
    </div>
	...
	$totalRows_selpic = mysql_num_rows($selpic);
        ...
        <div class="container6">
        	<div class="left6" style="width:590px;">
				<div><?php echo $row_rs_q_order['ptname']; ?></div>
				<div style="margin-top:-20px; color:#999999; font-size:30px;">แผนกหลัก : <?php echo $row_rs_q_order['department']; ?></div>
            </div>
  ...
  <?php
	...
	mysql_free_result($rs_q_order);
  	...
  	mysql_select_db($database_hos, $hos);
	$query_rs_q_order = "select main_dep_queue from ovst where vstdate=CURDATE() and cur_dep='".$row_channel['depcode']."' and cur_dep_time > '".$row_rs_max['cur_dep_time']."' order by cur_dep_time ASC limit 4";
  $rs_q_order = mysql_query($query_rs_q_order, $hos) or die(mysql_error());
	$row_rs_q_order = mysql_fetch_assoc($rs_q_order);
	$totalRows_rs_q_order = mysql_num_rows($rs_q_order);
