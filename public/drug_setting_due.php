...
$query_rs_drug = "select concat(name,strength) as drugname,icode from drugitems where istatus='Y' and icode not in (select icode from ".$database_kohrx.".kohrx_due_drug) order by name";
...
<title>Drug Use Evaluation</title>
...
$(document).ready(function(){
	...
	$('#save').click(function(){
                $('#indicator').show();
                $("#displayDiv").load('drug_setting_due_list.php?action=save&drug='+$('#drug').val(), function(responseTxt, statusTxt, xhr){
				...
				$('#indicator').show();
                $("#displayDiv").load('drug_setting_due_list.php', function(responseTxt, statusTxt, xhr){
...
function due_drug_delete(icode){

                $('#indicator').show();
                $("#displayDiv").load('drug_setting_due_list.php?action=delete&icode='+icode, function(responseTxt, statusTxt, xhr){
...
<div class="p-3">
	<div class="card">
		<div class="card-header">ตั้งค่า Drug use evaluate</div>
		<div class="card-body">
