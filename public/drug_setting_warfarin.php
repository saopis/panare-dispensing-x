...
if(isset($_POST['button8'])&&($_POST['button8']=="เพิ่ม"))
{
		...
		$query_rs_insert = "insert into ".$database_kohrx.".kohrx_drug_warfarin (icode,detail) value ('".$_POST['drug']."','".$_POST['detail']."')";
		$rs_insert = mysql_query($query_rs_insert, $hos) or die(mysql_error());

	//insert replicate_log
	mysql_select_db($database_hos, $hos);
	$update="insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','insert into ".$database_kohrx.".kohrx_drug_warfarin (icode,detail) value (\'".$_POST['drug']."\',\'".$_POST['detail']."\')')";
...
{
		mysql_select_db($database_hos, $hos);
		$query_rs_del = "delete from ".$database_kohrx.".kohrx_drug_warfarin where icode='".$_GET['icode']."'";
		$rs_del = mysql_query($query_rs_del, $hos) or die(mysql_error());
		
	//insert replicate_log
	mysql_select_db($database_hos, $hos);
	$update="insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','delete from ".$database_kohrx.".kohrx_drug_warfarin where icode=\'".$_GET['icode']."\'')";
	$qupdate= mysql_query($update, $hos) or die(mysql_error());

echo "<meta http-equiv=\"refresh\" content=\"0;URL=drug_setting_warfarin.php\" />";
exit();
}
if(isset($_GET['do'])&&($_GET['do']=="edit")){
mysql_select_db($database_hos, $hos);
$query_rs_edit = "SELECT * from ".$database_kohrx.".kohrx_drug_warfarin where icode='".$_GET['icode']."'";
...
{
mysql_select_db($database_hos, $hos);
$query_rs_edit = "update ".$database_kohrx.".kohrx_drug_warfarin set icode='".$_POST['drug']."' ,detail='".$_POST['detail']."'  where icode='".$_POST['icode']."'";
$rs_edit = mysql_query($query_rs_edit, $hos) or die(mysql_error());

	//insert replicate_log
	mysql_select_db($database_hos, $hos);
	$update="insert into replicate_log (`event_time`,`client`,`sql`) values (current_timestamp,'KOHRX','update ".$database_kohrx.".kohrx_drug_warfarin set icode=\'".$_POST['drug']."\' ,detail=\'".$_POST['detail']."\'  where icode=\'".$_POST['icode']."\'')";
	$qupdate= mysql_query($update, $hos) or die(mysql_error());

}
if(!isset($_GET['do'])&&($_GET['do']!="edit")){
mysql_select_db($database_hos, $hos);
$query_rs_drug3 = "SELECT icode,concat(name,strength) as drugname FROM s_drugitems WHERE icode like '1%' and istatus='Y' and icode not in (select icode from ".$database_kohrx.".kohrx_drug_warfarin) ORDER BY name ASC";
...
mysql_select_db($database_hos, $hos);
$query_rs_drug = "select p.icode,concat(d.name,' ',d.strength) as drugname,p.detail from ".$database_kohrx.".kohrx_drug_warfarin p left outer join s_drugitems d on d.icode=p.icode ORDER BY name ASC";
...
<title>รายการยาที่ห้ามสั่งใช้กับผู้ป่วยที่ใช้ยา Warfarin</title>
...
<div class="p-3">
<div class="card">
<div class="card-header"><h3>รายการยาที่ห้ามสั่งใช้กับผู้ป่วยใช้ Warfarin</h3></div>
<div class="card-body">
<form id="form1" name="form1" method="post" action="drug_setting_warfarin.php">
...
<div class="p-3">
  <table width="100%"  id="tables" class="table table-striped table-bordered table-sm" style="width:100%">
	<thead>
    <tr>
    ...
    <tr>
      ...
      <td align="center"><nobr><i class="fas fa-edit" style="color:#333; font-size:18px; cursor:pointer;" onclick="window.location='drug_setting_warfarin.php?do=edit&icode=<?php echo $row_rs_drug['icode']; ?>';" ></i>&nbsp;<i class="fas fa-trash" style="color:#333; font-size:18px; cursor:pointer;" onclick="if(confirm('ต้องการลบข้อมูลนี้จริงหรือไม่?')==true){window.location='drug_setting_warfarin.php?do=drug_delete&amp;icode=<?php echo $row_rs_drug["icode"]; ?>';}"></i></nobr></td>
    </tr>
