...
mysql_select_db($database_hos, $hos);
$query_rs_report = "select v.hn,concat(d.name,d.strength) as drugname,cc.use_cause,r.remark,c.name,".$condate." as vstdate from ".$database_kohrx.".kohrx_had_record r left outer join ".$condition1." left outer join doctor c on c.code=r.doctor left outer join drugitems d on d.icode=r.icode left outer join ".$database_kohrx.".kohrx_had_cause cc on cc.id=r.use_cause_id where ".$condate." between '".$_GET['datestart']."' and '".$_GET['dateend']."'".$condition;
